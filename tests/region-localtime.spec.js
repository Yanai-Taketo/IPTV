 ...
 *   Asia/Tokyo = 21:00 / Europe/Moscow = 15:00 / America/New_York = 08:00
 *
 * regions.json フィクスチャ: Asia(JP,RU)・North America(US)・
 * Empty Region(DE: チャンネル 0 → 選択肢に出ない)・WW/UN(常に除外)
 */
...
test.describe('地域・タイムゾーンデータが無い環境', () => {
  ...
  test('feeds.json が 404 なら現地時刻表示ごと消える(グレースフル)', async ({ page }) => {
    await routeApi(page);
